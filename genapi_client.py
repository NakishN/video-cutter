...
SUMMARY_PROMPT = """\
Ты — профессиональный редактор игрового и развлекательного видеоконтента. Твоя цель — выбрать ВСЕ интересные моменты для создания вирусных клипов (TikTok/Reels/Shorts).

Задача:
1. Выбери ВСЕ яркие, смешные, эмоциональные, эпичные или ключевые моменты из транскрипта. Не ограничивайся несколькими клипами, находи максимальное количество подходящих фрагментов.
...
Правила:
- Лимита на количество моментов нет. Постарайся найти как можно больше подходящих моментов (10, 20, 30 или даже больше).
...
- Отвечай на языке: {response_language}.
...
def summarize(
    ...
    *,
    ...
    prompt = SUMMARY_PROMPT.format(transcript=prepared, response_language=response_language)
    body = {
